...
fn draw_wiggly_text(){
    ...
    for i in 0..TEXT.len(){
        let init = Color::from_rgb((((micros as u32 / 10000).wrapping_sub(i as u32 * 16 )) % 255) as u8,255,255);
        let color = interface::sys::hsv_to_rgb(init.into()).into();
        draw_chacater(
            [i as u32*8 ,
            sin((i as u32*64+(micros as u32 / 1000 as u32)) % 1024) as u32 * 16 / 65536], TEXT[i] as u32, 
