...
impl From<u32> for Color{
    fn from(color: u32) -> Self {
        Color::from_rgb((color & 255) as u8, ((color >> 8) & 255) as u8, ((color >> 16) & 255) as u8)
    }
}


pub fn display_number(mut num: u32, location: impl Into<[u32; 2]>, leading_zeros: u32, forground: Color, background: Color) -> usize{
...
pub fn display_percentage<const D: u32>(top: u32, bottom: u32,  pos: impl Into<[u32; 2]>, forground: Color, background: Color){
    ...
    display_number(usage / pow(D), pos, 2, forground, background) as i16;
    pos[0] += 1; 
...
pub fn draw_string(string: &str, mut location: impl Into<[u32; 2]>, forground: Color, background: Color){
    let mut location = location.into();
    for char in string.as_bytes().iter(){
        let char = *char as char;
        let index = match char{
            'a'..='z' => {
                ((char as u32) - 'a' as u32) + 65
            }
            'A'..='Z' => {
                ((char as u32) - 'A' as u32) + 33
            }
            '0'..='9' => {
                char as u32 - '0' as u32 + 16
            }
            ' ' => {
                0
            }
...
pub fn draw_tiled_character(location: impl Into<[u32; 2]>, char: u32, forground: Color, background: Color){
    let mut location = location.into();
    location[1] *= 8;
    location[0] *= 8;
...
pub fn draw_chacater(location: impl Into<[u32; 2]>, char: u32, forground: Color, background: Color){
    ...
    for y in 0..8{
        ...
        for x in 0..8{
            ...
            if color.is_opaque(){
                ...
                interface::sys::set_pixel_coords(x + (location[0]), (y as u32) + (location[1]), color.into());
            } 
    ...
    for y in 0..8{
        ...
        macro_rules! x_thing {
            ($x:expr) => {
                ...
                if color.is_opaque(){
                    interface::sys::set_pixel_coords(($x<<1) + (location[0]), ((y as u32)<<1) + (location[1]), color.into());
                    interface::sys::set_pixel_coords(($x<<1) + 1 + (location[0]), ((y as u32)<<1) + (location[1]), color.into());
                    interface::sys::set_pixel_coords(($x<<1) + (location[0]), 1 + ((y as u32)<<1) + (location[1]), color.into());
                    interface::sys::set_pixel_coords(($x<<1) + 1 + (location[0]), 1 + ((y as u32)<<1) + (location[1]), color.into());
                } 
            };
        }
        x_thing!(0);
