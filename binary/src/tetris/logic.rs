...
use self::{game::TetrisGame, input::TetrisInput, renderer::TetrisRenderer, sound::TetrisSound};
...
mod game {
    ...
    impl TetrisGame {
        pub fn get_curr_piece(&self) -> Option<(u8, [Coord; 4])> {
            self.piece
                .as_ref()
                .map(|val| (val.piece_type.as_num(), val.get_coords()))
        }
    ...
    impl Tetris {
        pub fn update_game(&mut self) {
            ...
            if self.input.drop_down_pressed() {
                if let Some(dropped) = self.game.get_dropped_piece() {
                        ...
                        if piece.frames_since_last_fall > grav as usize {
                            ...
                            if self.game.board.is_any_intersecting(&coords) {
                                ...
                                for coord in piece.get_coords() {
                                    self.game
                                        .board
                                        .set_data_at_coord(piece.piece_type.as_num() + 1, coord);
                                }
                            }
            ...
            if new {
                if let Some(piece) = self.game.piece {
...
pub mod renderer {
    ...
    use crate::{
        tetris::{platform::Interface, logic::game::Coord, Color, InterfaceTrait},
        util::display::{display_number, display_percentage, draw_string, draw_tiled_character},
    };
    ...
    impl Tetris {
            ...
            if self.renderer.piece != curr {
                ...
                if let Option::Some(piece) = curr_dropped {
                    for coord in piece.1 {
                        self.ghost_cube(
                            coord - [-1, 19i16].into(),
                            TETROMINOE_PALLETE[piece.0 as usize][0],
                        )
                    }
                ...
                if let Option::Some(piece) = curr {
                    for coord in piece.1 {
                        self.draw_cube(
                            coord - [-1, 19i16].into(),
                            &TETROMINOE_PALLETE[piece.0 as usize],
                        )
                    }
                }
                        ...
                        for x in 0u8..10 {
                            ...
                            if t_data != t_old_data {
                                if t_data != 0 {
                                    self.draw_cube(
                                        [x + 1, y + 1].into(),
                                        &TETROMINOE_PALLETE[t_data as usize - 1],
                                    );
                                } else {
        ...
        fn ghost_cube(&mut self, location: Coord, color: Color) {
            ...
            for x in 0..8 {
                self.interface
                    .set_pixel(location.x as usize + x, location.y as usize, color);
                self.interface
                    .set_pixel(location.x as usize + x, location.y as usize + 7, color);
            }
            for y in 1..7 {
                self.interface
                    .set_pixel(location.x as usize, location.y as usize + y, color);
                self.interface
                    .set_pixel(location.x as usize + 7, location.y as usize + y, color);
            }
        }
        ...
        fn update_debug_info(&mut self, pos: Coord, forground: Color, background: Color) {
            ...
            }
            {
                ...
                if let Option::Some(frame_times) = self.debug.as_ref().unwrap().frame_times {
                    ...
                    pos.y += 1;
                    display_number(
                        frame_times.render_time as u32,
                        pos,
                        5,
                        forground,
                        background,
                    ...
                    if let Option::Some(render_times) = self.debug.as_ref().unwrap().render_times {
                        ...
                        pos.y += 1;
                        display_number(
                            render_times.background_time as u32,
                            pos,
                            5,
                            forground,
                            background,
                        ...
                        pos.y += 1;
                        display_number(
                            render_times.board_time as u32,
                            pos,
                            5,
                            forground,
                            background,
                        ...
                        pos.y += 1;
                        display_number(
                            render_times.pieces_time as u32,
                            pos,
                            5,
                            forground,
                            background,
            ...
            }
            {
                ...
                if let Option::Some(frame_times) = self.debug.as_ref().unwrap().frame_times {
                    display_percentage::<2>(
                        frame_times.input_time as u32,
                        frame_times.total_time as u32,
                        pos,
                        forground,
                        background,
                    );
...
mod input {
    ...
    impl Tetris {
        pub fn update_input(&mut self) {
            self.input
                .up
                .update(self.interface.key_down('w') | self.interface.key_down('\x26'));
            self.input
                .left
                .update(self.interface.key_down('a') | self.interface.key_down('\x25'));
            self.input
                .down
                .update(self.interface.key_down('s') | self.interface.key_down('\x28'));
            self.input
                .right
                .update(self.interface.key_down('d') | self.interface.key_down('\x27'));
